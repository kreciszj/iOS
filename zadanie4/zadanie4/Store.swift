...
final class Store: ObservableObject {
    ...
    func load() async {
        ...
        do {
            ...
            orders = o.sorted { $0.createdAt > $1.createdAt }

            saveToCoreData(categories: categories, products: products, orders: orders)
        } catch {
    ...
    func addProduct(name: String, price: Double, categoryId: Int) async {
        isLoading = true
        errorMessage = nil

        do {
            _ = try await api.createProduct(NewProduct(name: name, price: price, categoryId: categoryId))
            await load()
        } catch {
            errorMessage = (error as? LocalizedError)?.errorDescription ?? error.localizedDescription
            isLoading = false
    ...
    private func loadFromCoreData() {
        do {
            ...
            let loadedCategories: [Category] = catObjects.map { obj in
                let id = obj.value(forKey: "id") as? Int64 ?? 0
                let name = obj.value(forKey: "name") as? String ?? ""
                return Category(id: Int(id), name: name)
            }

            let loadedProducts: [Product] = prodObjects.map { obj in
                let id = obj.value(forKey: "id") as? Int64 ?? 0
                let name = obj.value(forKey: "name") as? String ?? ""
            ...
            let loadedOrders: [Order] = orderObjects.compactMap { obj in
                ...
                let items: [OrderItem] = itemsSet.map { it in
